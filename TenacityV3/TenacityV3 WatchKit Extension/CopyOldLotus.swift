...
class LotusController2: WKInterfaceController, WCSessionDelegate {
    ...
    @objc func reset(){
        ...
        if (current_round >= total_rounds){
            ...
            Main_Pic.setAlpha(0.0)
            seconds = 0
            //timer_reset = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector (LotusController2.counter_reset), userInfo: nil, repeats: true)
        }
    }
    ...
    @IBAction func TapAction(_ sender: Any) {
        if white{
            ...
            Main_Pic.setAlpha(0.0)
            randomize_color()
            timer_open = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector (LotusController2.counter_open), userInfo: nil, repeats: true)
        }
